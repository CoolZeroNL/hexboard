...
Container vizualization for link:http://openshift.com/[OpenShift platformV3]. 
...
[source, bash]
----
oc new-app -f https://raw.githubusercontent.com/CoolZeroNL/hexboard/master/app_template.json -p ACCESS_TOKEN=$(oc whoami -t)
----
...
[source, bash]
----
oc create -f https://raw.githubusercontent.com/CoolZeroNL/hexboard/master/app_template.json
----
...
[source, bash]
----
oc new-app -l servicegroup=sketchpod openshift/nodejs~http://github.com/CoolZeroNL/sketchpod
oc new-app -e "ACCESS_TOKEN=${ACCESS_TOKEN}" openshift/nodejs~http://github.com/CoolZeroNL/hexboard
----
...
You'll need to set the same token in your browser to reveal the Admin UI controls.  Visit "http://hexboard.tempdata.nl/?admin=YOUR_ADMIN_TOKEN" to add administrative UI controls to your browser.
...
After scaling up, try submitting sketches by visiting the hexboard's bundled mobile web submission form, at `http://hexboard.tempdata.nl/mobile`.
